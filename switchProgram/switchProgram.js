import shaders from './shader_framebuffer.js';
import negativeShader from './shader2.js';
...
const negativeProgram = util.initWebGL(gl, negativeShader.vertexShader, negativeShader.fragmentShader);
...
const negativeVariables = {
    a_position: gl.getAttribLocation(negativeProgram, 'a_position'),
    a_texCoord: gl.getAttribLocation(negativeProgram, 'a_texCoord'),
    u_canvas_size: gl.getUniformLocation(negativeProgram, 'u_canvas_size'),
    u_rotate: gl.getUniformLocation(negativeProgram, 'u_rotate'),
    u_projection: gl.getUniformLocation(negativeProgram, 'u_projection')
}
...
gl.enableVertexAttribArray(negativeVariables.a_position);
gl.vertexAttribPointer(negativeVariables.a_position, 2, gl.FLOAT, false, f32size * 2, 0);
gl.enableVertexAttribArray(negativeVariables.a_texCoord);
...
gl.enableVertexAttribArray(negativeVariables.a_texCoord);
gl.vertexAttribPointer(negativeVariables.a_texCoord, 2, gl.FLOAT, false, f32size * 2, 0);
gl.uniform2fv(negativeVariables.u_canvas_size, uniforms.u_canvas_size);
// gl.uniformMatrix4fv(negativeVariables.u_rotate, false, uniforms.u_rotate);
// gl.uniformMatrix4fv(negativeVariables.u_projection, false, uniforms.u_projection);
...
loadImages(['../assets/gaoda1.jpg', '../assets/gaoda2.jpg'])
    .then(([img1, img2]) => {
        ...
        // let originTexture2 = util.createTexture(gl);
