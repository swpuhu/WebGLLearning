...
function draw (context, text, offsetX, offsetY, lineHeightPx, width) {
    ...
    while (cursor + currentLength <= text.length) {
        ...
        let nextChar = text[cursor + currentLength];
        while (isBlank(nextChar) && cursor < text.length) {
            ++cursor;
        }
        if (isPunctuation(nextChar)) {
            let _width = context.measureText(str + nextChar).width;
            if (_width > width) {
                currentLength--;
...
window.drawText = function drawText (text, fontFamily = 'Microsoft YaHei', fontSize = 16, lineHeight = 0, fontStyle = 'normal', fontWeight = 'normal', textAlign = 'left', width = 100, height = 100, color = '#000', backgroundColor, shadowColor, url) {
    checkAndLoadFont(fontFamily, url).then(async (e) => {
        if (e && e.code === 0) {
            ...
            let actuallyFontSize = Math.floor(fontSize * coefficient);
            let lineHeightPx;
            if (!lineHeight) {
            ...
            let diffWinAscent = (Math.floor((winAscent - ascent) / unitsPerEm * fontSize));
            if (diffWinAscent < 0) {
                diffWinAscent = 0;
            }
